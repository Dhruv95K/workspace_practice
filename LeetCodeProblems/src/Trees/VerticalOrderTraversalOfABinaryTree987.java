...
public class VerticalOrderTraversalOfABinaryTree987 {
	...
	public List<List<Integer>> verticalTraversal(TreeNode root) {
		...
		Map<Integer, ArrayList<mapnode>> map = new TreeMap<>();
		Queue<mapnode> q = new LinkedList<>();
		q.offer(new mapnode(root, 0, 0));

		while (!q.isEmpty()) {
			mapnode t = q.poll();
			if (t != null) {
				if (map.get(t.hd) == null) {
					ArrayList<mapnode> l = new ArrayList<>();
					l.add(t);
					map.put(t.hd, l);
				} else
					map.get(t.hd).add(t);
				...
				if (t.node.left != null)
					q.offer(new mapnode(t.node.left, t.hd - 1, t.level + 1));

				if (t.node.right != null)
					q.offer(new mapnode(t.node.right, t.hd + 1, t.level + 1));

			}
		}

		for (int key : map.keySet()) {
			Collections.sort(map.get(key),new Comparator<mapnode>() {
				@Override
...
class mapnode {
	...
	public mapnode(TreeNode node, int hd, int level) {
		this.node = node;
		this.hd = hd;
		this.level = level;
	}
